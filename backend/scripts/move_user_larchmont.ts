...
// Larchmont target (within 50m)
const TARGET_CENTER = { latitude: 36.892703, longitude: -76.300946 };
const MIN_DISTANCE_METERS = 0;
const MAX_DISTANCE_METERS = 50;

const usage = () => {
  console.error("Usage: ts-node --transpile-only scripts/move_user_larchmont.ts user@example.com");
};
...
const main = async () => {
  ...
  console.log(
    `Moved ${label} near Larchmont -> ${coords.latitude.toFixed(6)}, ${coords.longitude.toFixed(6)} (${Math.round(distance)}m offset)`
  );
};

main()
  .catch((error) => {
    console.error("Failed to move user near Larchmont", error);
    process.exitCode = 1;
  })
  .finally(async () => {
